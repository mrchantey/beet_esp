//! Reflectable Alvik route handlers: the one-shot behaviours a scene can wire to
//! an HTTP path. Each is an `#[action(route)]` component a scene binds to a path
//! with the generic [`Route`](crate::scene::Route) widget, eg
//! `<Route path="drive/:dir" {DriveHandler}/>`. The hardware-agnostic
//! [`SpawnAction`] lives upstream with the generic scene server in [`beet::router`].
